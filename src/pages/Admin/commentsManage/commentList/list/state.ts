...
class State {
    ...
    selectCommentData = async () => {
        const res: any = await service.selectCommentData({
    ...
    deleteCommentData = async (id) => {
        const res: any = await service.deleteCommentData({ id });
        try{
            if( res.data.code === 200 ){
    ...
    addCommentData = async (values) => {
        const res: any = await service.addCommentData(values);
        try{
            if( res.data.code === 200 ){
                message.success( res.data.msg );
                this.selectCommentData();
    ...
    updateCommentData = async (values) => {
        const res: any = await service.updateCommentData(values);
        try{
            if( res.data.code === 200 ){
                message.success( res.data.msg );
                this.selectCommentData();
